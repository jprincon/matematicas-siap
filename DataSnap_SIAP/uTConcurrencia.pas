...
type
  ...
  private
    ...
    procedure seTConcurrencia(const Value: TFDConnection);
  ...
  published
    ...
    property Conexion: TFDConnection read FConexion write seTConcurrencia;
  end;
...
constructor TConcurrencia.create;
begin
  Query := TFDQuery.create(nil);
  FNombreTabla := 'math_visitas ';
  FSqlInsert := 'INSERT INTO ';
  FSqlSelect := 'SELECT * FROM ';
  FSqlUpdate := 'UPDATE ';
  FSqlDelete := 'DELETE FROM ';

  FSQLOrdenarPor := ' ORDER BY contador';

  FSQLClavePrincipal := 'id_visita';
  FJSONClavePrincipal := 'idVisita';

  FMensajePost := 'Visita creada';
  FMensajePut := 'Visita actualizada';
  FMensajeDelete := 'Visita eliminada';
  FMensajeSelect := 'Visitas obtenidas';

  FNombreLista := 'visitas';

  FPropiedadesTabla := TPropiedadesTabla.create;
  with FPropiedadesTabla do
  begin
    agregarPropiedad('id_visita');
    agregarPropiedad('contador');
  end;
end;

function TConcurrencia.delete(idClave: string): TJSONObject;
var
  Json: TJSONObject;
...
destructor TConcurrencia.destroy;
begin

end;

function TConcurrencia.get(idClave: string): TJSONObject;
var
  Json: TJSONObject;
  claveSQL, claveJSON, valor: string;
  i: integer;
...
function TConcurrencia.getAll: TJSONObject;
var
...
function TConcurrencia.model: TJSONObject;
var
  Json: TJSONObject;
  claveSQL, claveJSON, valor: string;
  i: integer;
...
function TConcurrencia.post(datos: TJSONObject): TJSONObject;
var
  Json: TJSONObject;
...
function TConcurrencia.put(datos: TJSONObject): TJSONObject;
var
  Json: TJSONObject;
  claveSQL, claveJSON: string;
  i: integer;
...
procedure TConcurrencia.seTConcurrencia(const Value: TFDConnection);
begin
  FConexion := Value;
  Query.Connection := Value;
end;

function TConcurrencia.where(nombre, valor: string): string;
begin
