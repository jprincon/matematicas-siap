...
type
  ...
  private
    ...
    procedure seTParticipante(const Value: TFDConnection);
  ...
  published
    ...
    property Conexion: TFDConnection read FConexion write seTParticipante;
  end;
...
begin
  ...
  FNombreTabla := 'math_participante_emem ';
  ...
  FMensajePost := 'Participante creado';
  FMensajePut := 'Participante actualizado';
  FMensajeDelete := 'Participante eliminado';
  FMensajeSelect := 'Participantes obtenidos';

  FNombreLista := 'participantes';
  ...
  begin
    ...
    agregarPropiedad('tipo_participacion');
    agregarPropiedad('afiliacion');
    agregarPropiedad('lugar_trabajo');
  end;
end;

function TParticipante.delete(idClave: string): TJSONObject;
var
  Json: TJSONObject;
...
function TParticipante.get(idClave: string): TJSONObject;
var
  Json: TJSONObject;
  claveSQL, claveJSON, valor: string;
  i: integer;
...
function TParticipante.getAll: TJSONObject;
var
...
function TParticipante.model: TJSONObject;
var
  Json: TJSONObject;
  claveSQL, claveJSON, valor: string;
  i: integer;
...
function TParticipante.post(datos: TJSONObject): TJSONObject;
var
  Json: TJSONObject;
...
function TParticipante.put(datos: TJSONObject): TJSONObject;
var
  Json: TJSONObject;
  claveSQL, claveJSON: string;
  i: integer;
...
procedure TParticipante.seTParticipante(const Value: TFDConnection);
begin
  FConexion := Value;
  Query.Connection := Value;
end;

function TParticipante.where(nombre, valor: string): string;
begin
